...
<div class="brand-form form-inline">
    ...
    <div class='col-md-12 col-sm-6 col-xs-12 left_padd'>
        <?= $form->field($model, 'banner_image')->fileInput()->label('Baner Image (File Size : 1920x903)') ?>
    ...
    <div class='col-md-12 col-sm-6 col-xs-12 left_padd'>
        <?= $form->field($model, 'collection_image')->fileInput()->label('Collection Image (File Size : 400x450)') ?>
        <?php if (isset($model->collection_image)) { ?>
            <img src="<?= Yii::$app->homeUrl ?>../uploads/cms/collections/<?= $model->id ?>/small.<?= $model->collection_image; ?>?<?= rand() ?>" width="300" height="110"/>

            <?php
        } elseif (!empty($model->collection_image)) {
            echo "";
        }
        ?>
    </div>
