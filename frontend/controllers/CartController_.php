...
class CartController extends \yii\web\Controller {
    ...
    public function actionBuynow() {
        if (yii::$app->request->isAjax) {
           ...
           $id = Product::find()->where(['canonical_name' => $canonical_name, 'status' => '1'])->one();
        if ($id) {
            ...
            $cart = Cart::find()->where(['product_id' => $id->id, 'item_type' => '0'])->andWhere($condition)->one();
            if (!empty($cart)) {
...
//        $this->redirect(['cart/proceed']);
        if (!isset(Yii::$app->user->identity->id)) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                $email = Cart::usercheck($model->email);
                if ($email) {
                    Cart::addGuest($model);
                } else {
                    $usererror = $model->email.' Allready Used';
                }
            }
//            var_dump($model);exit;
            return $this->render('checkout', ['model' => $model, 'usererror' => $usererror]);
        } else {
            $this->redirect(['cart/proceed']);
        }
    }
...
//            date_default_timezone_set('Asia/Kolkata');
    ...
    public function addOrder($cart) {
        ...
        if (isset(Yii::$app->user->identity->id)) {
            ...
            $ship_charge = $total_amt <= $shipping_limit ? sprintf('%0.2f', $shipextra) : 0.00;
            $model1->shipping_charge = $ship_charge;
            $model1->order_date = date('Y-m-d H:i:s');
            $model1->doc = date('Y-m-d');
...
public function Tax($orders) {


                $ordermaster = OrderMaster::findOne($orders['master_id']);

                $order_details = OrderDetails::find()->where(['master_id' => $ordermaster->id])->all();
                ...
                $ordermaster->tax_amount = $tax_amount;
              //  $ordermaster->net_amount = $ordermaster->net_amount + $tax_amount;
                $ordermaster->save();
        }
                ...
                if ($cart->item_type == 1) {
                    ...
                    if ($prod_details->offer_price == '0' || $prod_details->offer_price == '') {
                        ...
                        $price = $prod_details->offer_price;
                                        ...
                                        $tot_price = ($cart->quantity) * ($price);
                                        if (isset($prod_details->tax) && $prod_details->tax != '') {
                                                $tax_detail = \common\models\Tax::findOne($prod_details->tax);
                                                if (isset($tax_detail)) {
                                                        if ($tax_detail->type == 1) {
                                                                $tax = $tot_price * $tax_detail->value / 100;
                                                        } else if ($tax_detail->type == 2) {
                                                                $tax = $tot_price + $tax_detail->value;
                                                        }
                                                        $model_prod->tax_id = $tax_detail->id;
                                                }
                                        }
