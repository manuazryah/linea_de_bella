        ...
        public function actionConfirm_order() {
                if (isset(Yii::$app->user->identity->id)) {
                        if (Yii::$app->session['orderid']) {
                                ...
                                $model->status = 4;
                                if ($model->save()) {
                                         $this->CodeUsedSingle($model->id);
                                        //$this->stock_change();
//                    return $this->redirect(['payment', 'id' => $model->order_id]);
                                        //$this->sendMail($model->order_id);
...
//                    $this->redirect(array('site/index'));
       ...
       public function CodeUsedSingle($orderid) {
        ...
        public function actionPaypal($id) {/* action for creating pay page */
                ...
                $ship_address = UserAddress::findOne($model->ship_address_id);
        ...
        public function actionPaypalForm($url, $p_id, $order_id) {/* after creating paypage we get the url and action is used to redirect to the url */
                ...
                if ($model->update()) { /* save the ref id in order master table for comparing payment respose result */
                        Yii::$app->session['paypal-refernce-id'] = $p_id;
        ...
        public function actionPaymentResponse($ref_id) {/* after submitting the card details it will redirect to payment-response.php outer the frontend application from there it will redirect to current action */
                if (Yii::$app->session['paypal-refernce-id'] == $ref_id) {/* comparing the refernce id with the saved one */
                        $response = $ref_id;
        ...
        public function actionPaymentResult($result) {/* save the payment response */
                ...
                if ($result_data['response_code'] == 100) {/* 100 indicates transaction success */
                        if (Yii::$app->session['payment-order-id'] == $order_master->id) {
                               // $this->sendMail($order_master->order_id);
                                ...
                                $to = $mail;
                                $message = $this->renderPartial('mail', ['orderid' => $order_master->order_id]);
                                ...
                                Yii::$app->session['orderid'] = '';
                                Yii::$app->session['paypal-refernce-id'] = '';
                                Yii::$app->session['payment-order-id'] = '';
                                $model1 = Cart::find()->where(['user_id' => Yii::$app->user->identity->id])->all();
                                $this->purchasemail($order_master->order_id);
                                $this->clearcart($model1);
//				Yii::$app->session['datas'] = $charge;
                                return $this->redirect('payment-success');
                        }
                } else {
                        $order_master->payment_status = 0;
                        $order_master->payment_sucess_data = $result;

                        $order_master->status = 4; /* order confirmed for 4 */
                        $order_master->save();

                        $user = \common\models\User::findOne($order_master->user_id);
                        $message = Yii::$app->mailer->compose('payment-fail', ['user' => $user, 'orderid' => $order_master->order_id])
        ...
        public function purchasemail($orderid) {
        ...
        <body>
        <p>New Purchase Order with Order Id :- <b>" . $orderid . "</b></p>
        <table>
        <tr>
        <th>Order Id</th>
        <th>:-</th>
        ...
        <th>Amount</th>
        <th>:-</th>
        <td>AED " . sprintf('%0.2f', $ordermaster->net_amount) . "</td>
        </tr>
                        ...
                        if ($order_details->net_amount == $_POST['order_amount']) {
                                ...
                                try {
                                        ...
                                        if (!empty($charge)) {
                                                ...
                                                Yii::$app->session['orderid'] = '';
                                                $model1 = Cart::find()->where(['user_id' => Yii::$app->user->identity->id])->all();
                                                $this->purchasemail(Yii::$app->session['orderid']);
                                                $this->clearcart($model1);
                                                Yii::$app->session['datas'] = $charge;
                                                return $this->redirect('payment-success');
                                        } else {
                                ...
                                if ($date_check == 1) {
                                        ...
                                        if ($used == 0) {
                                                ...
                                                if ($exist == 1) {
                                                        ...
                                                        if ($code_exists->promotion_type == 1) {
                                                                echo '4';
                                                        } else if ($code_exists->promotion_type == 2) {
                                                                echo '7';
                                                        } else {
                                                                echo '8';
                                                        }
...
// Always set content-type when sending HTML email
                ...
                $headers .= 'From: no-replay@coralperfumes.com' . "\r\n";
$headers .= "To: $to\r\n";
...
public function sendMail1($orderid) {

                $mail = \common\models\User::findOne(Yii::$app->user->identity->id)->email;
                $message = Yii::$app->mailer->compose('mail', ['orderid' => $orderid]) // a view rendering result becomes the message body here
                        ->setFrom('no-replay@coralperfumes.com')
                        ->setTo($mail)
                        ->setSubject('Order Confirm');
